...
const RED       = '#BF1A1A', NAVY = '#060771', GREEN = '#16a34a'
const ORANGE    = '#ea580c', AMBER = '#b45309', PURPLE = '#7c3aed'
const FUNNEL_C  = { 'on process':NAVY,'mcu':AMBER,'accepted':GREEN,'rejected':RED,'mengundurkan diri':ORANGE }
...
function Recruitment() {
  ...
  useEffect(() => {
    ...
    fetch(API_URL).then(r => r.json()).then(j =>
      setOptions({ posisi: (j.posisi||[]).map(d => cleanVal(d.posisi)) })
    ).catch(() => {})
  }, [])

  const setF   = (k,v) => setFilters(p => ({ ...p, [k]:v }))
  const apply  = () => fetchData(filters)
  const reset  = () => { const e={status:'',posisi:'',startMonth:'',endMonth:''}; setFilters(e); fetchData(e) }

  if (loading) return <div className="rc-wrapper"><div className="rc-loading"><div className="rc-spinner"/><p>Memuat data...</p></div></div>
  if (error)   return <div className="rc-wrapper"><div className="rc-error"><p>Gagal: {error}</p><button onClick={() => fetchData()}>Coba lagi</button></div></div>
  if (!data)   return null
  ...
  const cleanPosisi  = (posisi||[]).map(d => ({ ...d, posisi: cleanVal(d.posisi) }))
  const cleanFunnel  = (funnel||[]).map(d => ({ ...d, status: cleanVal(d.status), color: FUNNEL_C[d.status]||NAVY }))
  ...
  return (
    <div className="rc-wrapper">
      ...
      <div className="rc-filter-bar">
        <div className="rc-filter-inner">
          <div className="rc-filter-group">
            ...
            <select className="rc-filter-select" value={filters.status} onChange={e => setF('status', e.target.value)}>
          ...
          <div className="rc-filter-group">
            ...
            <select className="rc-filter-select" value={filters.posisi} onChange={e => setF('posisi', e.target.value)}>
          ...
          <div className="rc-filter-group">
            ...
            <input type="month" className="rc-filter-select" value={filters.startMonth} onChange={e => setF('startMonth', e.target.value)}/>
          </div>
          <div className="rc-filter-divider"/>
          <div className="rc-filter-group">
            <label className="rc-filter-label">Periode Akhir</label>
            <input type="month" className="rc-filter-select" value={filters.endMonth} onChange={e => setF('endMonth', e.target.value)}/>
          </div>
        </div>
        <div className="rc-filter-actions">
          <button className="rc-btn-reset" onClick={reset}>Reset</button>
          <button className="rc-btn-apply" onClick={apply}>Terapkan</button>
        </div>
      </div>
      ...
      {/* TREN — row mandiri, height lebih kecil */}
      ...
      {/* ROW: Kandidat per Posisi (kiri) + Distribusi Status (kanan) */}
      <div className="rc-row-2" style={{ gridTemplateColumns:'1.4fr 1fr' }}>

        {/* Posisi hbar + toggle top */}
        <div className="rc-card">
          <div className="rc-card-header">
            ...
            <div className="rc-top-toggle">
              {['5','10','all'].map(v => (
                <button key={v} className={`rc-top-btn${topPosisi===v?' rc-top-btn--active':''}`}
        ...
        <div className="rc-card">
          ...
          {pieStatus.length === 0 ? <p className="rc-no-data">Tidak ada data</p> : (
            <>
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={pieStatus} cx="50%" cy="50%"
                    innerRadius={52} outerRadius={78} paddingAngle={3} cornerRadius={4}
