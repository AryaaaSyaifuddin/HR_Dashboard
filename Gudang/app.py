...
def load_recruitment():
    try:
        ...
        # Baca semua sheet names dari Excel
        xl = pd.ExcelFile(FILE_PATH)
        all_sheets = xl.sheet_names
        
        # Filter sheet yang mengandung kata "RECRUITMENT" (case-insensitive)
        ...
        # Baca semua sheet recruitment
        df_list = []
        total_rows = 0
        
        for sheet in recruitment_sheets:
            try:
                # Baca sheet dengan kolom B:E
                ...
                # Tambah kolom untuk tracking source sheet (optional)
                df['source_sheet'] = sheet
                
                # Clean data per sheet sebelum digabung
                df = df.dropna(subset=['nama', 'posisi'], how='all')  # Hapus row kosong total
                df['nama'] = df['nama'].astype(str).str.strip()
                df = df[df['nama'] != 'nan']  # Filter row dengan nama 'nan'
        ...
        # 3. Parse periode (tanggal)
        df['periode'] = pd.to_datetime(df['periode'], errors='coerce')
        
        # 4. Buat kolom bulan untuk grouping
        df['bulan'] = df['periode'].dt.strftime('%B %Y')
        
        # 5. Drop rows yang tidak punya nama atau posisi
...
def calculate_recruitment_kpi(df):
    """Calculate KPI for recruitment dashboard"""
    if df.empty:
        return {
            "total": 0,
            "accepted": 0,
